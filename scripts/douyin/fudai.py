...
class Tee(object):
    ...
    def write(self, obj):
        for f in self.files:
            ...
            # 格式化行号信息
            line_info = f"[{caller_filename}:{caller_line}] "
            # 如果文本不是空字符串，则添加行号信息
            if obj.strip():
                obj = line_info + obj
            f.write(obj)
...
class fudai_analyse:
    ...
    def __init__(self):
        ...
        self.base_resolution_x = 1080
        self.base_resolution_y = 2400
        self.resolution_ratio_x = 1080
        self.resolution_ratio_y = 2240
        timepic = datetime.now().strftime('%Y-%m-%d-%H-%M')
        log_file = open(os.path.dirname(__file__) +
                        '/log/'+timepic+'.log', 'w', encoding='utf-8')
        sys.stdout = Tee(sys.stdout, log_file)
        self.last_find_fudai_time = 0.0
        self.tesseract_cmd = None
        self.configure_tesseract_cmd(silent=True)

    def scale_x(self, x):
        """将1080基准横坐标缩放到当前设备宽度"""
        return x * self.resolution_ratio_x // self.base_resolution_x

    def scale_y(self, y):
        """将2400基准纵坐标缩放到当前设备高度"""
        return y * self.resolution_ratio_y // self.base_resolution_y

    def update_device_width(self):
        """自动识别设备屏幕宽度，兼容不同宽度的屏幕"""
        try:
            string = subprocess.Popen(
                "adb -s %s shell wm size" % self.device_id,
                shell=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE
            )
            totalstring = string.stdout.read().decode('utf-8')
            result = re.search(r'(\d+)x(\d+)', totalstring)
            if result:
                self.resolution_ratio_x = int(result.group(1))
                print("自动识别设备分辨率宽度:{}".format(self.resolution_ratio_x))
                return True
        except Exception as e:
            print("自动识别设备宽度失败:{}".format(e))
        print("未识别到设备宽度，使用默认宽度:{}".format(self.resolution_ratio_x))
        return False

    def configure_tesseract_cmd(self, silent=False):
        """自动查找 tesseract.exe，优先使用 PATH 中的可执行文件"""
        candidate_paths = []
        path_tesseract = shutil.which('tesseract')
        if path_tesseract:
            candidate_paths.append(path_tesseract)
        for drive in ('D', 'E', 'F', 'C'):
            candidate_paths.append(
                '{}:/Program Files/Tesseract-OCR/tesseract.exe'.format(drive))
            candidate_paths.append(
                '{}:/Program Files (x86)/Tesseract-OCR/tesseract.exe'.format(drive))
        for candidate in candidate_paths:
            if candidate and os.path.exists(candidate):
                pytesseract.pytesseract.tesseract_cmd = candidate
                self.tesseract_cmd = candidate
                if not silent:
                    print("当前使用的 Tesseract 路径:{}".format(candidate))
                return candidate
        raise pytesseract.pytesseract.TesseractNotFoundError(
            "未找到 tesseract.exe，请安装 Tesseract-OCR 或把它加入 PATH。"
        )
    ...
    def save_reward_pic(self):
        ...
        subprocess.Popen(
            'adb  -s %s pull /sdcard/DCIM/screenshot.png %s/%s.png ' % (
                self.device_id, path, timepic),
            stdout=subprocess.PIPE).wait()
        subprocess.Popen(
            'adb  -s %s shell rm /sdcard/DCIM/screenshot.png' % self.device_id).wait()
    ...
    def cut_pic(self, left_up=(0, 63), right_down=(1080, 1620), target='', name='', resolution=(1080, 2400)):
        ...
        if target == '' or target == False:
            ...
            pic.crop((left_up[0], left_up[1], right_down[0],
                     right_down[1])).save(cut_pic_path)
            return True
    ...
    def analyse_pic_word(self, picname='', type=1, change_color=True):
        ...
        if not self.tesseract_cmd or not os.path.exists(self.tesseract_cmd):
            self.configure_tesseract_cmd()
        if type != 2:
            text = pytesseract.image_to_string(img, lang='chi_sim')
        # text = pytesseract.image_to_string(img, lang='eng+chi_sim')
        else:
            # MedianFilter 将每个像素的值替换为其周围像素值的中值，以减少图像中的噪声。
        ...
        if type == 2:
            ...
            text = ''.join(
                [char for char in text if char.isnumeric()])  # 针对时间去噪
    ...
    def check_robot_pic_distance(self):
        """处理人机验证的图片"""
        self.cut_pic((self.scale_x(143), 884), (self.scale_x(936), 1380), '', 'cut')
        path = os.path.dirname(__file__) + '/pic'
        pic = path + '/cut.png'
        img = Image.open(pic)
        img = img.convert('RGB')
                    ...
                    for dx in range(-1, 2):
                        for dy in range(-1, 2):
                            if 0 <= x + dx < width and 0 <= y + dy < height:  # 确保不超出图像边界
                                ...
                                # 检查颜色格式
                                if isinstance(neighbor_color, tuple) and len(neighbor_color) == 3:
                                    neighbor_color = tuple(
                                        # 确保颜色值在0到255之间
                                        min(max(int(c), 0), 255) for c in neighbor_color)
                                    channel_diffs = [
                                        abs(a - b) for a, b in zip(current_color, neighbor_color)]
    ...
    def check_countdown(self, last_time=''):
        ...
        try:
            ...
            formatted_future_time = future_datetime.strftime(
                '%Y-%m-%d %H:%M:%S')
    ...
    def check_detail_height(self):
        ...
        pix = pic_new.load()
        if 30 <= pix[self.scale_x(536), 883*self.resolution_ratio_y//2400][0] <= 38 and 34 <= pix[self.scale_x(536), 883*self.resolution_ratio_y//2400][1] <= 40 and 78 <= pix[self.scale_x(536), 883*self.resolution_ratio_y//2400][2] <= 84:
            print('参与抽奖有3个任务')
            return 3
        elif 30 <= pix[self.scale_x(536), 983*self.resolution_ratio_y//2400][0] <= 38 and 34 <= pix[self.scale_x(536), 983*self.resolution_ratio_y//2400][1] <= 40 and 78 <= pix[self.scale_x(536), 983*self.resolution_ratio_y//2400][2] <= 84:
            print('参与抽奖有2个任务')
            return 2
        elif 30 <= pix[self.scale_x(536), 1058*self.resolution_ratio_y//2400][0] <= 38 and 34 <= pix[self.scale_x(536), 1058*self.resolution_ratio_y//2400][1] <= 40 and 78 <= pix[self.scale_x(536), 1058*self.resolution_ratio_y//2400][2] <= 84:
            print('参与抽奖有1个任务')
            return 1
    ...
    def check_have_fudai(self):
        ...
        while loop < 6:  # 每3秒识别一次，最多等待18秒
            ...
            pic_new = pic.convert('RGBA')
            pix = pic_new.load()
            expected_y = self.scale_y(403) + self.y_pianyi
            search_top = max(self.scale_y(330), expected_y - self.scale_y(50))
            search_bottom = min(pic.height - 1,
                                expected_y + self.scale_y(25))
            matched_points = []
            for y in range(search_top, search_bottom + 1):
                for x in range(self.scale_x(41), self.scale_x(410)):
                    if 194 <= pix[x, y][0] <= 200 and 187 <= pix[x, y][1] <= 193 and 241 <= pix[x, y][2] <= 247:
                        matched_points.append((x, y))
            if len(matched_points) >= 3:
                self.last_find_fudai_time = time.time()
                fudai_x = min(point[0] for point in matched_points)
                fudai_y = min(point[1] for point in matched_points)
                print("检测到福袋图标区域，x={}, y={}".format(fudai_x, fudai_y))
                return fudai_x
            loop += 1
            if loop >= 4:
                self.deal_robot_analyse()
    ...
    def check_have_robot_analyse(self):
        """检查是否存在人机校验"""
        self.cut_pic((self.scale_x(130), 790*self.resolution_ratio_y//2400), (self.scale_x(680), 870 *
                     self.resolution_ratio_y//2400), '', 'zhibo_yanzheng')  # 福袋内容详情
    ...
    def deal_swipe_robot_analyse(self, distance=400):
        """处理滑动图片的人机验证"""
        if distance:
            targetx = self.scale_x(222) + distance
        else:
            targetx = self.scale_x(622)
        os.system("adb -s %s shell input swipe %s %s %s %s 300" %
                  (self.device_id, self.scale_x(222), self.scale_y(1444), targetx, self.scale_y(1444)))
    ...
    def reflash_zhibo(self):
        ...
        os.system("adb -s %s shell input swipe %s %s %s %s 200" %
                  (self.device_id, self.scale_x(760), self.scale_y(800), self.scale_x(760), self.scale_y(1600)))
        time.sleep(5)

    def check_in_follow_list(self):
        """判断是否界面在我的关注的列表页"""
        self.cut_pic((self.scale_x(244), 137*self.resolution_ratio_y//2400), (self.scale_x(875), 220 *
                     self.resolution_ratio_y//2400), '', 'zhibo_follow_list')  # 福袋内容详情
    ...
    def check_in_zhibo_list(self):
        """检查是否当前在直播列表"""
        self.cut_pic((self.scale_x(400), 145*self.resolution_ratio_y//2400), (self.scale_x(675), 230 *
                     self.resolution_ratio_y//2400), '', 'zhibo_list_title')  # 福袋内容详情
    ...
    def check_zhibo_is_closed(self):
        """检查当前直播间是否关闭"""
        self.cut_pic((self.scale_x(350), 200*self.resolution_ratio_y//2400), (self.scale_x(740), 300 *
                     self.resolution_ratio_y//2400), '', 'zhibo_status')  # 福袋内容详情
    ...
    def check_zhibo_is_closed_guess_whatyoulike(self):
        ...
        self.cut_pic((self.scale_x(440), 1570*self.resolution_ratio_y//2400), (self.scale_x(640),
                     1640*self.resolution_ratio_y//2400), '', 'zhibo_status')  # 福袋内容详情
        zhibo_list_title = self.analyse_pic_word('zhibo_status', 1)
        print(zhibo_list_title)
    ...
    def back_to_zhibo_list(self):
        ...
        while click_back_times < 4:
            ...
            if self.check_in_zhibo_list():
                ...
                # todo check20241023
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          self.scale_x(119), 397 * self.resolution_ratio_y // 2400))
                # os.system("adb -s {} shell input tap 390 {}".format(self.device_id,
                #                                                     550 * self.resolution_ratio_y // 2400))
                print("点击打开直播间的列表")
                time.sleep(2)
                return False
            os.system("adb -s %s shell input keyevent 4" % self.device_id)
    ...
    def into_zhibo_from_list(self):
        ...
        while True:
            ...
            if self.check_in_zhibo_list():
                ...
                if 3 < current_hour <= 6:
                    ...
                    if current_hour > 8:  # 如果当前时间已经早上9点多了，一定有直播间了
                        os.system("adb -s {} shell input tap {} {}".format(
                            self.device_id, self.scale_x(290), 490*self.resolution_ratio_y//2400))  # 点击第一个直播间
                        print("点击打开第一个直播间")
                        break  # 跳出循环，直播间已找到
                        ...
                        os.system("adb -s %s shell input keyevent 4" %
                                  self.device_id)
                        time.sleep(3)
                        print("点击退出到关注列表")
            elif self.check_zhibo_have_popup():
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          self.scale_x(540), 1620*self.resolution_ratio_y//2400))
                print("点击关闭红包弹窗")
                os.system("adb -s %s shell input keyevent 4" % self.device_id)
                time.sleep(3)
                print("点击退出直播间")
                ...
                # os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                #           45, 440*self.resolution_ratio_y//2400))  # 点击直播中
                ...
                print("当前页面不在直播间，默认关注列表")
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          self.scale_x(119), 397 * self.resolution_ratio_y // 2400))
                time.sleep(5)  # 等待10秒钟继续检查
                print("等待5秒钟后再检查")
    ...
    def check_zhibo_list_have_zhibo(self):
        ...
        pic_new = pic.convert('RGBA')
        pix = pic_new.load()
        if pix[self.scale_x(290), 490*self.resolution_ratio_y//2400][0] == 255 and pix[self.scale_x(290), 490*self.resolution_ratio_y//2400][1] == 255 and pix[self.scale_x(290), 490*self.resolution_ratio_y//2400][2] == 255:
            print('直播间列表为空')
            return False
        print('直播间列表存在直播的内容')
        return True
    ...
    def check_zhibo_have_popup(self):
        """判断直播间是否弹出了节假日红包弹窗"""
        self.cut_pic((self.scale_x(425), 880*self.resolution_ratio_y//2400), (self.scale_x(660), 960 *
                     self.resolution_ratio_y//2400), '', 'zhibo_hongbao')  # 福袋内容详情
    ...
    def get_fudai_contain(self, renwu=2):
        """获取福袋的内容和倒计时"""
        print("福袋共{}个任务".format(renwu))
        if renwu == 1:  # 如果是1个任务的
            self.cut_pic((self.scale_x(390), 1300*self.resolution_ratio_y//2400 + self.y_pianyi), (self.scale_x(1000),
                         1470*self.resolution_ratio_y//2400 + self.y_pianyi), '', 'fudai_content')  # 福袋内容详情
            self.cut_pic((self.scale_x(390), 1190*self.resolution_ratio_y//2400), (self.scale_x(690), 1280 *
                         self.resolution_ratio_y//2400), '', 'fudai_countdown')  # 完整福袋详情倒计时
        elif renwu == 2:  # 如果是2个任务的
            self.cut_pic((self.scale_x(390), 1240*self.resolution_ratio_y//2400 + self.y_pianyi), (self.scale_x(1000),
                         1410*self.resolution_ratio_y//2400 + self.y_pianyi), '', 'fudai_content')  # 福袋内容详情
            self.cut_pic((self.scale_x(397), 1120*self.resolution_ratio_y//2400), (self.scale_x(690), 1210 *
                         self.resolution_ratio_y//2400), '', 'fudai_countdown')  # 完整福袋详情倒计时
        elif renwu == 3:  # 如果是3个任务的
            self.cut_pic((self.scale_x(390), 1160*self.resolution_ratio_y//2400 + self.y_pianyi), (self.scale_x(1000),
                         1340*self.resolution_ratio_y//2400 + self.y_pianyi), '', 'fudai_content')  # 福袋内容详情
            self.cut_pic((self.scale_x(390), 1020*self.resolution_ratio_y//2400), (self.scale_x(690), 1110 *
                         self.resolution_ratio_y//2400), '', 'fudai_countdown')  # 完整福袋详情倒计时
        else:
            self.cut_pic((self.scale_x(390), 1392*self.resolution_ratio_y//2400 + self.y_pianyi), (self.scale_x(1000),
                         1570*self.resolution_ratio_y//2400 + self.y_pianyi), '', 'fudai_content')  # 福袋内容详情
            self.cut_pic((self.scale_x(390), 1275*self.resolution_ratio_y//2400), (self.scale_x(690), 1370 *
                         self.resolution_ratio_y//2400), '', 'fudai_countdown')  # 完整福袋详情倒计时
    ...
    def attend_choujiang(self, renwu=1, fudai_x=0):
        """点击参与抽奖"""
        click_times = 0
        while click_times < 2:
            self.cut_pic((self.scale_x(306), 2140*self.resolution_ratio_y//2400), (self.scale_x(780), 2280 *
                         self.resolution_ratio_y//2400), '', "attend_button")  # 参与福袋抽奖的文字
            ...
            if "参与成功" in attend_button_text:  # 如果识别到已经参与抽奖
                ...
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          self.scale_x(500),
                          440*self.resolution_ratio_y//2400))  # 点击刚才打开福袋的旁边位置
                print("点击福袋外部，关闭福袋详情")
                return True
                ...
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          self.scale_x(500),
                          2210*self.resolution_ratio_y//2400))  # 点击参与抽奖
                print("点击参与抽奖")
                return True
                ...
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          self.scale_x(500),
                          2210*self.resolution_ratio_y//2400))  # 点击加入粉丝团、点亮粉丝团
                time.sleep(2)
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          self.scale_x(500),
                          440*self.resolution_ratio_y//2400))  # 点击刚才打开福袋的旁边位置
                ...
                time.sleep(1)
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          fudai_x + self.scale_x(45), 440*self.resolution_ratio_y//2400))  # 点击刚才打开小福袋的位置
    ...
    def check_have_reward(self):
        ...
        pix = pic_new.load()
        y = 1238
        if 253 <= pix[self.scale_x(540), 1238*self.resolution_ratio_y//2400][0] <= 255 and 43 <= pix[self.scale_x(540), 1238*self.resolution_ratio_y//2400][1] <= 45 and 84 <= pix[self.scale_x(540), 1238*self.resolution_ratio_y//2400][2] <= 86:
            y = 1238
        elif 253 <= pix[self.scale_x(540), 1290*self.resolution_ratio_y//2400][0] <= 255 and 43 <= pix[self.scale_x(540), 1290*self.resolution_ratio_y//2400][1] <= 45 and 84 <= pix[self.scale_x(540), 1290*self.resolution_ratio_y//2400][2] <= 86:
            y = 1290
        self.cut_pic((self.scale_x(306), (y+20)*self.resolution_ratio_y//2400), (self.scale_x(780),
                     (y+110)*self.resolution_ratio_y//2400), '', "get_reward")  # 立即领取奖品
    ...
    def check_have_reward_notice_confirm(self):
        ...
        pic_new = pic.convert('RGBA')
        pix = pic_new.load()
        self.cut_pic((self.scale_x(370), 1350 * self.resolution_ratio_y // 2400),
                     # 提醒领取奖品的弹窗
                     (self.scale_x(680), 1440 * self.resolution_ratio_y // 2400), '', "notice_reward")
    ...
    def close_no_reward_popup(self):
        """关闭未中奖弹窗，避免误点到上方的券入口"""
        self.cut_pic((self.scale_x(280), 1450 * self.resolution_ratio_y // 2400),
                     (self.scale_x(800), 1700 * self.resolution_ratio_y // 2400), '', "no_reward_confirm")
        confirm_text = self.analyse_pic_word('no_reward_confirm', 1)
        print("未中奖弹窗确认区域文字：{}".format(confirm_text))
        tap_y = 1580 * self.resolution_ratio_y // 2400
        os.system("adb -s {} shell input tap {} {}".format(
            self.device_id, self.scale_x(540), tap_y))
        print("点击未中奖弹窗确认按钮")
        time.sleep(2)
    ...
    def get_reward(self, reward_y=0):
        ...
        os.system(
            # 勾选协议
            "adb -s {} shell input tap {} {}".format(self.device_id, self.scale_x(243), reward_y))
        time.sleep(1)
        os.system("adb -s {} shell input tap {} {}".format(self.device_id, self.scale_x(540),
                  reward_y - 140*self.resolution_ratio_y//2400))  # 点击领取
        print("勾选协议，点击领取奖品")
        time.sleep(10)
        # self.save_reward_pic()
        os.system(
            # 点击下单
            "adb -s {} shell input tap {} {}".format(self.device_id, self.scale_x(886), 2170 * self.resolution_ratio_y // 2400))
        print("点击下单")
        time.sleep(10)
        os.system("adb -s %s shell input keyevent 4" %
                  self.device_id)  # 下完单点击返回直播间
        print("下完单点击返回直播间")
        time.sleep(10)
        if self.check_have_reward():
            print("领奖弹窗未关闭，点击关闭弹窗")
            os.system("adb -s {} shell input tap {} {}".format(self.device_id, self.scale_x(540),
                      reward_y + 180*self.resolution_ratio_y//2400))  # 勾选协议
            time.sleep(2)
    ...
    def fudai_choujiang(self, device_id="", y_pianyi=0, y_resolution=2400, needswitch=False, wait_minutes=15):
        ...
        while True:
            ...
            if x:
                ...
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          x + self.scale_x(45), 440*self.resolution_ratio_y//2400))  # 点击默认小福袋的位置
                print("点击打开福袋详情")
                time.sleep(3)
            ...
            if result:
                ...
                os.system(
                    "adb -s %s shell input swipe %s %s %s %s 200" % (self.device_id, self.scale_x(760), self.scale_y(1600), self.scale_x(760), self.scale_y(800)))
                continue
            if lastsecond < 10 and needswitch:  # 如果不到15秒了，就不点了
                os.system("adb -s {} shell input tap {} {}".format(self.device_id,
                          x + self.scale_x(45), 440*self.resolution_ratio_y//2400))  # 点击刚才打开小福袋的位置
                print("点击小福袋位置，关闭福袋详情")
                time.sleep(1)
                if needswitch:
                    os.system(
                        "adb -s %s shell input swipe %s %s %s %s 200" % (self.device_id, self.scale_x(760), self.scale_y(1600), self.scale_x(760), self.scale_y(800)))
                    print("抽奖倒计时时间小于10秒，不参与，上划切换直播间")
                    swipe_times += 1
                time.sleep(5)
                continue
            if needswitch and lastsecond >= 60*wait_minutes:  # 如果需要切换且倒计时时间大于设定的分钟
                ...
                time.sleep(5)
                continue
            if not self.attend_choujiang(renwu, x):  # 如果参与抽奖失败
                os.system(
                    "adb -s %s shell input swipe %s %s %s %s 200" % (self.device_id, self.scale_x(760), self.scale_y(1600), self.scale_x(760), self.scale_y(800)))
            ...
            self.get_screenshot()
            self.cut_pic((self.scale_x(357), 658*self.resolution_ratio_y//2400), (self.scale_x(740), 750 *
